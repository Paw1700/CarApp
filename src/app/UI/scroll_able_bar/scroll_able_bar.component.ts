...
export class ScrollAbleBar implements OnInit {
    @Input() first_option = new ScrollBarOption('', 25, null, null)
    @Input() second_option = new ScrollBarOption('', 25, null, null)
    @Input() first_option_return_value: any = null
    @Input() second_option_return_value: any = null
    @Input() listen_to_scroll = true
    ...
    private readonly diffXLimit = 50
            ...
            switch (type) {
                ...
                case "end":
                    ...
                    if (Math.abs(this.start_touch_position.y - this.end_touch_posistion.y) < this.diffYLimit) {
                        if (this.start_touch_position.x - this.end_touch_posistion.x > this.diffXLimit) { // SWIPE RIGHT
                            if (this.bar_posistion === this.bar_defualt_position && this.second_option.text !== '') {
                                this.bar_posistion = this.bar_second_position
                            } else {
                                this.bar_posistion = this.bar_defualt_position
                            }
                        } else if (this.start_touch_position.x - this.end_touch_posistion.x < -this.diffXLimit) { // SWIPE LEFT 
                            if (this.bar_posistion === this.bar_defualt_position && this.first_option.text !== '') {
